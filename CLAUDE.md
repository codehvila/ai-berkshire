...
- **工具输出的代码块内只能放工具真实打印的内容。** 附录（Anexo）是可复现的审计轨迹：
  读者重跑同一条命令必须得到同样的块。自行推导的计算（EV/EBITDA、P/S、CAGR、FCF yield、
  PE 的各种口径等）必须放在代码块之外，或在块内逐行标注 `— derivado, no salida de herramienta`。
  （既有报告不追溯修改，仅适用于新报告。）
